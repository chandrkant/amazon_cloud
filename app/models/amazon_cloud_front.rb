class AmazonCloudFront < ApplicationRecord
  def self.fetch_api
    resp = {}
  ...
  def self.price_on_demand_data_insert
    _all_data = fetch_api
    _data = _all_data['terms']['OnDemand']
    _keys = _data.keys
    _keys.each do |_key|
  ...
  def self.response_data(date = nil,region='india')
    _attrs = 'description, begin_range as beginRange, end_range as endRange, unit, price_per_unit as pricePerUnit,effective_date as effectiveDate'
    if date
       data = self.select(_attrs).where('date(effective_date)=? && region=?',date.to_date,region.downcase)
    else
       data = self.select(_attrs).where('region=?',region.downcase)
    end
